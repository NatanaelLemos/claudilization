...
- 2026-08-20 — Gauntlet rounds 1+2 deployed to Fly from branch `gauntlet-aaa-visuals` @ `dba1997` (release v56, bundle `assets/index-DJqmNPHG.js` — the standalone build of that commit reproduces the served hashes exactly). Round's critique and the fixed-camera capture script are now in the repo. Pre-deploy backup `backups/claudilization-production-pre-gauntlet-20260820T165500Z.json` (114 islands, 6724 log rows, 16.5 MB). Live checks: https://claudilization.com serves the new bundle, `/api/world` returns 114 islands, and a `wss://claudilization.com/ws` session took 652 frames (26 world, 626 island) in which all 41 distinct creation designs crossed the wire carrying voxel models, none sprite-only, zero client errors. Branch pushed, not merged — `main` is Nate's call.
- 2026-08-20 — Gauntlet round 2 (the critic's three headline fixes): ground is painted from three tonal zones chosen where height and slope argue — damp hollow, working meadow, dry bleached crown — with three octaves of value noise at 11/26/58 tiles breaking the boundaries into patches, every threshold stretched over the range each island actually occupies, and `amplifyTerrainNormals` tilting the normals so the live sun models a miniature hill that only subtends three degrees; `contactShadows.ts` gives every tree, building and plaza a contact disc that carries falloff in vertex alpha and blends as `dst * (1 - a)`, so it keeps the ground's hue in daylight and vanishes at night, with canopies baking their own underside shade; and foam is banded by distance to the waterline instead of depth, which kills the orphan rings that hung offshore wherever the seabed flattened. `npm run verify` green (594 pass, 12 new). Branch `gauntlet-aaa-visuals` @ `32a4c7d`, not deployed. Frame: `notes/evidence/2026-08-20-gauntlet-r2-desktop.png`.
- 2026-08-20 — Gauntlet round 1 (two visual workstreams vs Townscaper): terrain now carries baked sky occlusion — `terrainSkyOcclusion` walks eight horizons plus a wide-ring bowl/dome term per vertex at mesh time and folds it into the ground's existing vertex colours (free at render, inherited by the distant LOD), closing the "no baked AO" gap the notes have carried since wave 3; and the sea's foam ring stopped being a constant-width gasket — a very low-frequency coast field varies exposure round each island and the deep banks down past the lagoon. Blind critic still calls Townscaper the winner, not close. `npm run verify` green (582 pass). Branch `gauntlet-aaa-visuals` @ `4c812d6`, not deployed. Frames: `notes/evidence/2026-08-20-gauntlet-{before,r1,r2}-desktop.png`.
- 2026-08-19 — Operator repair: `scripts/prune-buildings.mjs` (parameterized island/type/count, `--dry`, refuses a snapshot that does not round-trip byte-for-byte and refuses to strand a referenced building id). Applied to the live world with the machine stopped — Little Saint James (`island-130`) went from 302 stacked `elder-lodge` to 151 (5559 → 5408 buildings, newest ordinals b585–b949 dropped); `world_log` untouched, backup `backups/claudilization-production-pre-prune-20260819T221629Z.json`.
- 2026-08-18 — Wave 9 (solid creations): every invented thing is a 3D voxel model, greedy-meshed into clay geometry that stands, turns, and casts a shadow; flat sprites are carved into relief at the gate and on load, so nothing renders 2D again; models cross the wire once per viewer and creations take posts on a spiral; the rulebook, `/claudilization`, and the self-update's skill refresh all teach the format. See `notes/2026-08-18-wave9-solid-creations.md`. Deployed to Fly (bundle `ce561cbcc6a0`); the live world carved its 37 existing designs on load — every design now crosses the wire as a model, none as a sprite, and the renderer reports zero billboards (`notes/evidence/2026-08-18-live-solid-creations*.png`).
